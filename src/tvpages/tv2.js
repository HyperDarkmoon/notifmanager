...
function TV2() {
  ...
  const [pressure, setPressure] = useState(1008.50);
  ...
  useEffect(() => {
    const fetchCustomContent = async () => {
      try {
        // Fetch active schedules for TV2 from the backend
        console.log('TV2 - Attempting to fetch content from backend');
        
        const schedules = await debugAuthenticatedApiCall('http://localhost:8090/api/content/tv/TV2', {
          method: 'GET'
        });
        ...
        if (schedules) {
          ...
          console.log(`TV2 - Active schedules:`, activeSchedules);
          ...
          if (prevContentStr !== newContentStr) {
            console.log(`TV2 - Content changed, updating state`);
            setCustomContent(activeSchedule || null);
            prevContentRef.current = activeSchedule;
          }
        } else {
          console.error('Failed to fetch content for TV2');
          setCustomContent(null);
        }
        
        // Fallback to localStorage for development
        const currentUploads = JSON.parse(localStorage.getItem('tvUploads') || '{}');
        const tv2Content = currentUploads['2'];
        
        if (tv2Content && !schedules) {
          console.log(`TV2 - Using localStorage fallback:`, tv2Content);
          
          // Compare with previous content to see if it changed
          const prevContentStr = JSON.stringify(prevContentRef.current);
          const newContentStr = JSON.stringify(tv2Content);
          
          if (prevContentStr !== newContentStr) {
            console.log(`TV2 - LocalStorage content changed, updating state`);
            setCustomContent(tv2Content || null);
            prevContentRef.current = tv2Content;
          }
        }
      } catch (error) {
        ...
        if (tv2Content) {
          console.log(`TV2 - Using localStorage fallback due to error:`, tv2Content);
          const prevContentStr = JSON.stringify(prevContentRef.current);
          const newContentStr = JSON.stringify(tv2Content);
  ...
  useEffect(() => {
    ...
    console.log(`TV2 - Content count: ${contentCount}, Custom content present: ${Boolean(customContent)}`);
    ...
    const interval = setInterval(() => {
      setContentIndex(prevIndex => {
        ...
        console.log(`TV2 - Rotating content: ${prevIndex} -> ${nextIndex}, Content types: ${contentCount}`);
        return nextIndex;
      });
    }, rotationPeriod);
  ...
  const renderContent = () => {
    console.log(`TV2 - Rendering content index: ${contentIndex}, Has custom content: ${Boolean(customContent)}`);
